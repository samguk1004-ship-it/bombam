...
io.on('connection', (socket) => {
    ...
    socket.on('submitOffer', ({ roomCode, targetId, card, claim }) => {
        const room = rooms[roomCode];
        room.activeOffer = {
            card, claim, senderId: socket.id, receiverId: targetId, seenIds: [socket.id]
        };
        room.phase = 'RESPONSE';
    ...
    socket.on('resolveResponse', ({ roomCode, guessIsTrue }) => {
        ...
        const wasAttackSuccessful = (guessIsTrue !== actualIsTrue);
        const loserId = wasAttackSuccessful ? offer.receiverId : offer.senderId;

        room.phase = 'REVEAL';
        io.to(roomCode).emit('revealStart', { room, loserId, wasAttackSuccessful });
    ...
    // [5] 패스 (Pass)
    socket.on('submitPass', ({ roomCode }) => {
        const room = rooms[roomCode];
        const currentReceiver = room.players.find(p => p.id === socket.id);
        room.activeOffer.senderId = socket.id;
        room.activeOffer.seenIds.push(socket.id);
        room.phase = 'PASS_TARGET';
        io.to(roomCode).emit('passState', room);
    });

    // 보안을 위해 개인별 핸드 정보만 따로 전송하는 함수
    function updatePrivateHands(roomCode) {
        const room = rooms[roomCode];
    ...
    socket.on('disconnect', () => { /* 방에서 제거 로직 */ });
});
