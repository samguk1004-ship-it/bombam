...
flip7Io.on('connection', (socket) => {
    socket.on('joinRoom', ({ roomCode, userName, userId, isBot }) => {
        ...
        if (!room.players.find(p => p.id === socket.id)) {
            room.players.push({ id: socket.id, userId, name: userName, isBot, ready: room.players.length === 0, score: 0 });
        }
    ...
    socket.on('disconnect', () => { for (const roomCode in flip7Rooms) leaveFlip7Room(socket, roomCode); });
});
